...
st.markdown(
    '<div class="main-title">Google Reviews Sentiment Analyzer</div>',
    unsafe_allow_html=True
)

st.markdown(
    '<div class="subtitle">Fetch Google Maps reviews and analyze customer sentiment</div>',
    unsafe_allow_html=True
)
        ...
        try:
            ...
            try:
                ...
                if reviews_button.count() > 0:
                    reviews_button.click(timeout=15000)
                else:
...
if st.button("Fetch Reviews and Analyze"):
    if not business_name:
        ...
        with st.spinner("Fetching Google reviews in hidden browser mode..."):
            reviews = fetch_google_reviews(business_name)

        if not reviews:
            st.error(
                "No reviews found. Google Maps may block hidden browser mode. "
                "Try again or temporarily use headless=False for testing."
            )
            st.stop()
